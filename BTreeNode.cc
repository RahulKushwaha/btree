...
void writeNonLeafNode(node_id_t childNodeId, const std::string& key,
  ...
  nonLeafKeyValue.keyLength =
      reinterpret_cast<length_t*>(baseAddr + sizeof(node_id_t));
  nonLeafKeyValue.key =
      reinterpret_cast<char*>(baseAddr + sizeof(node_id_t) + sizeof(length_t));
...
void writeLeafNode(const std::string& key, const std::string& value,
  ...
  leafKeyValue.valueLength =
      reinterpret_cast<length_t*>(baseAddr + sizeof(length_t) + key.length());
...
NonLeafKeyValue readFromNonLeafNode(void* block, length_t blockSize) {
  ...
  NonLeafKeyValue nonLeafKeyValue{
      ...
      .key = reinterpret_cast<char*>(baseAddr + sizeof(node_id_t) +
                                     sizeof(length_t)),
  };
...
LeafKeyValue readFromLeafNode(void* block, length_t blockSize) {
  ...
  leafKeyValue.valueLength = reinterpret_cast<length_t*>(
      baseAddr + sizeof(length_t) + *leafKeyValue.keyLength);
  leafKeyValue.value = reinterpret_cast<char*>(
      baseAddr + sizeof(length_t) + *leafKeyValue.keyLength + sizeof(length_t));
...
BTreeNode::BTreeNode(BufferPageControl* bufferPageControl)
    : bufferPageControl_{bufferPageControl},
      btreeNodeHeader_{reinterpret_cast<btree_node_header_t*>(
          bufferPageControl_->getSubHeaderLocation().ptr)},
      mtx_{std::make_shared<std::mutex>()} {
  assert(bufferPageControl && "bufferpage control cannot be null");
}

std::pair<BTreeNode::Result, data_location_t> BTreeNode::insertNonLeaf(
    node_id_t childNode, const std::string& key) {
...
std::pair<BTreeNode::Result, data_location_t> BTreeNode::insertLeaf(
...
void BTreeNode::eraseAll() {
  ...
  assert(bufferPageControl_->getBufferPage()->header.data_list_size == 0 &&
         "datalist should be empty now");
}
...
data_location_t BTreeNode::search(std::string key) {
  ...
  auto result = std::upper_bound(
      dataList.begin(), dataList.end(), key,
      [&locationLookup, this](const auto& x, const auto& y) {
...
void BTreeNode::copyAllDataBlocks(BTreeNode* dest) {
  ...
  for (auto dataBlock : dataDictionaryList) {
    ...
    if (isLeaf()) {
      ...
      dest->insertNonLeaf(*nonLeafKeyValue.childNodeId,
                          nonLeafKeyValue.getKeyStr());
    }
  }
}
